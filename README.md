...
```
   Tokens (B × T)
        │
        ▼
   Token Embedding (64K × 768, tied with LM head)
        │
        ▼
   ┌─────────── 24× FusionLLMBlock (alternating MLA ↔ GDN) ───────────┐
   │                                                                  │
   │   RMSNorm → Attention (MLA | GDN) → (+) → RMSNorm → FFN → (+)   │
   │                 ↑                            ↑                   │
   │           (residual)                   (residual)                │
   │                                                                  │
   └──────────────────────────────────────────────────────────────────┘
        │
        ▼
   Final RMSNorm → LM Head (tied, logit softcap ±15.0) → logits
                                          │
                                          ▼
                                    MTP Heads (depth 2, 3)
                                    λ₂=0.10, λ₃=0.05
...
```
  ...
  MLA MLA GDN MLA MLA GDN MLA MLA GDN MLA MLA GDN MLA MLA GDN MLA MLA GDN MLA MLA GDN MLA MLA GDN

  16× MLA+MoE   ·   8× GDN+dense SwiGLU   (interleaved)
```
...
```
  ...
  Attention = MLA (Q LoRA 192, KV rank 96, decoupled RoPE)  |  Gated Delta Net (32 heads)
  FFN       = DeepSeekMoE (8 routed top-2 + 1 shared, aux-loss-free biased-sigmoid)
              | dense SwiGLU
```
...
```
   1. Download          2. Preprocess         3. Tokenizer (64K BPE)
   ─────────────        ──────────────        ────────────────────────
   FineWeb-Edu  55%     text clean            trained from scratch
   FineWeb      20%     language filter       (tiktoken-compatible)
   Stack Py     10%
   SlimPajama    8%     ────────────          ────────────────────────
   Dolma Wiki    4%           │                       │
   Dolma Books   3%           ▼                       ▼
